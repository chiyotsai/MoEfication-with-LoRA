...
def change_forward(model, result_path, k=20):
    """Modify the forward pass so that we only take the top k experts' output."""
    ...
    for layer_idx, layer in enumerate(model.encoder.block):
        ...
        path = os.path.join(result_path, 'param_split', 'encoder.block.{}.layer.1.DenseReluDense.wi.weight'.format(layer_idx))
        modify_ffn(ffn, path) 

    #decoder
    for layer_idx, layer in enumerate(model.decoder.block):
        ffn = layer.layer[2].DenseReluDense
        path = os.path.join(result_path, 'param_split', 'decoder.block.{}.layer.2.DenseReluDense.wi.weight'.format(layer_idx))
        modify_ffn(ffn, path)    
