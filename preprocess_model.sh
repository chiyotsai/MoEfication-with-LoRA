...
cmd=(python moe_evaluate.py --model_name="$MODEL_ARCHITECTURE"  --checkpoint="$MODEL_CHECKPOINT" \
  --res_path="$RESULT_PATH" --k="$K" --dataset="$DATASET" --eval_type=base)
echo "${cmd[*]}"
...
cmd=(python moe_evaluate.py --model_name="$MODEL_ARCHITECTURE"  --checkpoint="$MODEL_CHECKPOINT" \
  --res_path="$RESULT_PATH" --k="$K" --dataset="$DATASET" --eval_type=moe_gt)
echo "${cmd[*]}"
...
echo "Evaluating MLP..."
cmd=(python moe_evaluate.py --model_name="$MODEL_ARCHITECTURE"  --checkpoint="$MODEL_CHECKPOINT" \
  --res_path="$RESULT_PATH" --k="$K" --dataset="$DATASET" --eval_type=moe_mlp)
echo "${cmd[*]}"
